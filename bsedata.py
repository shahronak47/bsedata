...
def get_current_stock_prices(share_list, bse) :
    ...
    for share in share_list :
        ...
        #If the code is valid and we have the data take the 1st index, 2nd is difference from yesterday
        if current_data is not None :
            current_data_list.append(current_data[0])
            diff_list.append(current_data[1])
        else :
            #If we do not have the data we append 0
            current_data_list.append(0)
            diff_list.append(0)

    return current_data_list, diff_list
...
if __name__ == '__main__' :
    ...
    current_data_list, diff_list = get_current_stock_prices(df['Shares'].tolist(), bse)
